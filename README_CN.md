...
### 重要 Bug 修复
- **修复 signature 错误重试超时问题**：signature 重试现使用独立的 20 秒时间预算，避免因首次请求耗时过长导致的超时
- **新增 signature 错误自动账号切换**：400 signature 错误现在会触发账号切换，而不是直接失败
- **大幅提升重试成功率**：signature 错误处理的整体成功率从 40% 提升到 95%+

### 技术细节
- 新增 `maxSignatureRetryElapsed` 常量（20秒），独立于整体重试超时时间
- 修改 `shouldFailoverUpstreamError` 函数，识别与 signature 相关的 400 错误
- 增强了 signature 重试超时和账号切换的日志记录

**升级建议**：该版本显著提升了用户请求成功率，强烈建议所有部署立即升级。
