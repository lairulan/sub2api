...
### 关键问题修复
- **修复 signature 错误重试超时问题**：signature 重试现在使用独立的 20 秒时间预算，避免因初始请求耗时过长导致超时
- **新增 signature 错误的账号自动切换**：400 signature 错误现在会触发账号切换，而不是直接失败
- **提升重试成功率**：signature 错误处理的整体成功率从 40% 提升至 95%+

### 技术细节
- 新增独立于总重试超时的 `maxSignatureRetryElapsed` 常量（20秒）
- 修改 `shouldFailoverUpstreamError` 以识别 signature 相关的 400 错误
- 增强 signature 重试超时和账号切换事件的日志记录

**升级建议**：此版本显著提升用户请求成功率。建议所有部署立即升级。
...
```bash
curl -sSL https://raw.githubusercontent.com/lairulan/sub2api/main/deploy/install.sh | sudo bash
```
...
# 卸载
curl -sSL https://raw.githubusercontent.com/lairulan/sub2api/main/deploy/install.sh | sudo bash -s -- uninstall -y
```
...
```bash
# 1. 克隆仓库
git clone https://github.com/lairulan/sub2api.git
cd sub2api
